...
设计阶段，**决策已全部落定**（ADR-0001 ~ 0007），准备冻结基线。
...
- [ ] **技术验证（spike）**：第三方扩展能否在 Electron + pi SDK 里加载（`.ts` 运行时出口是第一个要验的）
- [ ] 按决策改写 [architecture.md](./design/architecture.md) 第 3.3 / 3.4 节
- [ ] 把设计文档里 `【待决】` 的地方逐条落实（含权限规则格式）
- [ ] 需求文档补 `REQ-x.y` 编号，填完追溯矩阵
- [ ] V1 范围定案（[02-scope-v1.md](./02-scope-v1.md) 第三节）
- [ ] 设计评审 → 冻结 v1.0 基线
