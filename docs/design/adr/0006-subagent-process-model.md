...
> **决策摘要**：**不自己实现子代理**，复用社区成熟扩展——选定 **`pi-subagents`（nicobailon）**，生态下载量第一（428,818/月），决策人本人在深度使用。
> **理由**：与 ADR-0007 一致——避免重复造轮子，复用社区实现以减少工作量。
> **因此本 ADR 的角色从"选一个方案自己做"变成"选一个包并接好它"**：下面的调研结论仍然全部有效，但用途从"实现指南"转为"选型依据 + 集成注意事项"。
>
> 📌 本 ADR 正文保留了完整的调研过程。**早期草稿的倾向（自研调度器 + 借鉴 gotgenes）已被推翻**，以第「决策（已接受）」节为准。

> ⚠️ **这条不拍板，架构文档 3.3 节和权限闸的"主/子共用同一道闸"就写不实。**
> 注意：架构文档 v0.1 第 3.3 节和决策 D2 曾写"subagent 是独立 pi 进程"——**那是在没有验证 pi 官方 subagent 扩展的前提下写的，已被本 ADR 推翻。**（architecture.md 已按本 ADR 改写为 v0.2）
...
- [x] 架构文档 3.3 节按本条结论重写（2026-09-15 完成，见 [architecture.md](../architecture.md) v0.2）
