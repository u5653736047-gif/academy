...
### 负面 / 代价
- ⚠️ **本闸不是抗篡改的安全边界**：扩展链上的 `event.input` 可变且改后不重新校验，后加载的 handler 能覆盖先加载的判定（夸张地说：另一个扩展可以"把闸拆了"）。**这是选择"复用社区扩展"换取工作量的代价，已明确接受。**
- **受包的版本节奏牵制**：`@gotgenes/pi-permission-system` 迭代极快（4 个月 210 个版本、有破坏性变更），升级时要复测
- 闸自身的未捕获异常会导致工具被拒（fail-closed 是好事），但要**能区分"闸坏了"和"用户拒了"**，否则给学生误导性提示

### 需要后续跟进的事
- [ ] **扩展加载自检**：扩展加载失败是**静默**的（`loader.ts` 只 push 进 `errors[]`），必须自己断言"闸真的装上了"，否则拒绝启动
- [ ] 定义审批请求 / 回执的字段（见 [ipc-contract.md](../ipc-contract.md)）
- [ ] 明确"闸异常"与"用户拒绝"在 GUI 上的不同呈现
- [ ] **锁精确版本**（不用 `^` / `~`），并记录当前锁定版本
- [ ] `permission-gate.md` 按本决策重写（v0.1 还残留"两层串联 / 包装 `Agent.beforeToolCall`"的旧方案描述）
